...
create index if not exists messages_conversation_created_at on public.messages (conversation_id, created_at);
...
create table if not exists public.artifacts (
  ...
  status text not null default 'uploaded' check (status in ('uploaded', 'normalized', 'failed')),
  normalized_artifact_id uuid references public.artifacts(id) on delete set null,
...
create table if not exists public.usage_events (
  ...
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  agent_run_id uuid references public.agent_runs(id) on delete set null,
  kind text not null check (kind in ('agent_run', 'storage', 'connector')),
...
create table if not exists public.collection_artifacts (
  ...
  artifact_id uuid not null references public.artifacts(id) on delete cascade, primary key(collection_id, artifact_id)
);
create table if not exists public.knowledge_chunks (
  ...
  ordinal integer not null check (ordinal >= 0), content text not null, start_offset integer not null check (start_offset >= 0), end_offset integer not null check (end_offset >= start_offset),
  unique(artifact_id, ordinal)
);
create table if not exists public.research_reports (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null references public.workspaces(id) on delete cascade,
  title text not null check (char_length(title) between 1 and 255), content text not null, citations jsonb not null default '[]'::jsonb,
  artifact_id uuid references public.artifacts(id) on delete set null, created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now()
);
create table if not exists public.adapter_registrations (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null references public.workspaces(id) on delete cascade,
  name text not null check (name in ('graphify','graft','headroom','agent_reach')), manifest jsonb not null, enabled boolean not null default false,
  created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now(), unique(workspace_id,name)
);
create table if not exists public.skill_observations (
  id uuid primary key default gen_random_uuid(), workspace_id uuid not null references public.workspaces(id) on delete cascade,
  title text not null, manifest jsonb not null, provenance jsonb not null default '{}'::jsonb,
  status text not null default 'draft' check(status in ('draft','dismissed','accepted')), created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now()
);
...
drop policy if exists "members manage workspace artifacts" on public.artifacts;
drop policy if exists "members manage knowledge collections" on public.knowledge_collections;
drop policy if exists "members access collection artifacts" on public.collection_artifacts;
drop policy if exists "members access knowledge chunks" on public.knowledge_chunks;
drop policy if exists "members manage research reports" on public.research_reports;
drop policy if exists "members read adapters" on public.adapter_registrations;
drop policy if exists "admins manage adapters" on public.adapter_registrations;
drop policy if exists "members manage skill observations" on public.skill_observations;
create policy "members manage workspace artifacts" on public.artifacts for all using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=artifacts.workspace_id and organization_memberships.user_id=(select auth.uid()))) with check (created_by=(select auth.uid()) and exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=artifacts.workspace_id and organization_memberships.user_id=(select auth.uid())));
create policy "members manage knowledge collections" on public.knowledge_collections for all using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=knowledge_collections.workspace_id and organization_memberships.user_id=(select auth.uid()))) with check (created_by=(select auth.uid()) and exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=knowledge_collections.workspace_id and organization_memberships.user_id=(select auth.uid())));
create policy "members access collection artifacts" on public.collection_artifacts for all using (exists (select 1 from public.knowledge_collections join public.workspaces on workspaces.id=knowledge_collections.workspace_id join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where knowledge_collections.id=collection_artifacts.collection_id and organization_memberships.user_id=(select auth.uid()))) with check (exists (select 1 from public.knowledge_collections join public.workspaces on workspaces.id=knowledge_collections.workspace_id join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where knowledge_collections.id=collection_artifacts.collection_id and organization_memberships.user_id=(select auth.uid())));
create policy "members access knowledge chunks" on public.knowledge_chunks for all using (exists (select 1 from public.artifacts join public.workspaces on workspaces.id=artifacts.workspace_id join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where artifacts.id=knowledge_chunks.artifact_id and organization_memberships.user_id=(select auth.uid()))) with check (exists (select 1 from public.artifacts join public.workspaces on workspaces.id=artifacts.workspace_id join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where artifacts.id=knowledge_chunks.artifact_id and organization_memberships.user_id=(select auth.uid())));
create policy "members manage research reports" on public.research_reports for all using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=research_reports.workspace_id and organization_memberships.user_id=(select auth.uid()))) with check (created_by=(select auth.uid()) and exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=research_reports.workspace_id and organization_memberships.user_id=(select auth.uid())));
create policy "members read adapters" on public.adapter_registrations for select using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=adapter_registrations.workspace_id and organization_memberships.user_id=(select auth.uid())));
create policy "admins manage adapters" on public.adapter_registrations for insert with check (created_by=(select auth.uid()));
create policy "members manage skill observations" on public.skill_observations for all using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id=workspaces.organization_id where workspaces.id=skill_observations.workspace_id and organization_memberships.user_id=(select auth.uid()))) with check (created_by=(select auth.uid()));
...
drop policy if exists "users add own messages" on public.messages;
...
create policy "workspace admins create delegated runs" on public.agent_runs
  for insert with check (
    parent_run_id is not null and requested_by = (select auth.uid()) and exists (
      select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id = workspaces.organization_id
      where workspaces.id = agent_runs.workspace_id and organization_memberships.user_id = (select auth.uid())
        and organization_memberships.role in ('owner', 'platform_admin', 'workspace_admin')
    )
  );
create policy "members read workspace retention policies" on public.workspace_retention_policies
  for select using (exists (
    select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id = workspaces.organization_id
    where workspaces.id = workspace_retention_policies.workspace_id and organization_memberships.user_id = (select auth.uid())
  ));
create policy "workspace admins manage retention policies" on public.workspace_retention_policies
  for all using (exists (
    select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id = workspaces.organization_id
    where workspaces.id = workspace_retention_policies.workspace_id and organization_memberships.user_id = (select auth.uid())
      and organization_memberships.role in ('owner', 'platform_admin', 'workspace_admin')
...
create policy "members read workspace usage" on public.usage_events
  for select using (exists (
    select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id = workspaces.organization_id
    where workspaces.id = usage_events.workspace_id and organization_memberships.user_id = (select auth.uid())
  ));
...
create trigger audit_workspace_created
...
create trigger audit_membership_changed
...
create trigger audit_approval_changed
...
create trigger audit_skill_changed
...
create trigger enforce_skill_immutability
...
create trigger audit_connector_changed
  after insert or update on public.connectors
...
create trigger audit_agent_run_changed
...
create trigger audit_policy_changed
...
create trigger audit_custom_role_changed
...
create trigger audit_custom_role_assignment_changed
  after insert or update on public.custom_role_assignments
  for each row execute function public.audit_custom_role_changed();
...
create policy "users read own messages" on public.messages
  for select using (exists (select 1 from public.conversations where id = conversation_id and user_id = (select auth.uid())));

create policy "users add own messages" on public.messages
  for insert with check (exists (select 1 from public.conversations where id = conversation_id and user_id = (select auth.uid())));
...
create index if not exists external_connections_workspace_created_at on public.external_connections (workspace_id, created_at desc);
...
create policy "members read workspace external connections" on public.external_connections
  for select using (exists (select 1 from public.workspaces join public.organization_memberships on organization_memberships.organization_id = workspaces.organization_id where workspaces.id = external_connections.workspace_id and organization_memberships.user_id = (select auth.uid())));
...
begin
  select organization_id into event_organization_id from public.workspaces where id = new.workspace_id;
  insert into public.audit_events (organization_id, workspace_id, actor_id, action, resource_type, resource_id, details)
  values (event_organization_id, new.workspace_id, auth.uid(), 'external_connection.' || case when tg_op = 'INSERT' then 'requested' else new.status end, 'external_connection', new.id, jsonb_build_object('provider', new.provider, 'scopes', new.scopes, 'status', new.status));
  return new;
end;
$$;
drop trigger if exists audit_external_connection_changed on public.external_connections;
create trigger audit_external_connection_changed after insert or update on public.external_connections for each row execute function public.audit_external_connection_changed();
