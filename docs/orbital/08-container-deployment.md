...
Orbital runs as four containers: `web` (port 8080), `gateway` (port 3000), `api` (internal port 8000), and `orbital-runners` (internal port 8010). Supabase is intentionally external and configured through `server/.env`.
